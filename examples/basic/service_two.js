...
const myConfig = {
  serviceName: 'Streaming', // A name to easily identify the service being monitored
  updateIntervalSeconds: 2, // Time interval, in seconds, between updates
  role: 'service', // Role can either be 'service' or 'registry'
  groupingKey: 'StreamingService', // A grouping key so all the services of the same type are hold together
...
app.get('/stream', (req, res) => {
  logger('Got request to /stream');
  res.send('Hello I am a mock stream service');
});

const expressApp = app.listen(() => logger('Mock Service 2 listening on port: ', expressApp.address().port));
